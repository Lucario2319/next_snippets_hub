...
export const GlowArea = ({ size = 100, className, ...props }: GlowAreaProps) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // ensure framerate matches device refresh rate to prevent extra renders
    if (!frameId.current){
...
export const Glow = ({ color = 'teal', className, children, ...props }: GlowProps) => {
  const element = useRef<HTMLDivElement>(null);
